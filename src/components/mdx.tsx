      ...
      [
        createElement(
          "a",
          {
            ...
            key: `link-${slug}`,
            className: "anchor",
            "aria-label": `Lien vers la section : ${headingText}`,
          },
          createElement(
            "span",
            {
            ...
            `Lien vers la section : ${headingText}`
          )
        ),
      ],
      children
...
function ProsCard({ title, pros }: { title: string; pros: string[] }) {
  return (
    <div className="border border-emerald-200 dark:border-emerald-900 bg-emerald-50 dark:bg-neutral-900 rounded-xl p-6 my-4 w-full">
      <span className="text-neutral-900 dark:text-neutral-100">{`Vous pouvez utiliser ${title} si...`}</span>
      <div className="mt-4">
        {pros.map((pro: string) => (
